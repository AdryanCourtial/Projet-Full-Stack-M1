...
export const updateBudgetService = async (userId: number, id: number, dto: UpdateBudgetDto) => {
    ...
    return prisma.budget.update({
        ...
        data: {
            ...
            ...(dto.amountPlanned !== undefined ? { amountPlanned: dto.amountPlanned } : {}),
...
export const deleteBudgetService = async (userId: number, id: number) => {
    ...
    const txCount = await prisma.transaction.count({ where: { userId, budgetId: id } });
    if (txCount > 0) {
        const err: any = new Error("Cannot delete a budget that is used by transactions");
        err.statusCode = 409;
        throw err;
    }
...
export const addBudgetCategoriesService = async (userId: number, budgetId: number, dto: AddBudgetCategoriesDto) => {
    ...
    if (missing.length > 0) {
        const err: any = new Error(`Some categories were not found: ${missing.join(", ")}`);
        err.statusCode = 404;
        throw err;
    }
...
export const removeBudgetCategoryService = async (userId: number, budgetId: number, categoryId: number) => {
    const budget = await prisma.budget.findFirst({ where: { id: budgetId, userId }, select: { id: true } });
    if (!budget) {
        const err: any = new Error("Budget not found");
        err.statusCode = 404;
