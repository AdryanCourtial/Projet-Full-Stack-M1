...
const addMonthsClampedUTC = (date: Date, monthsToAdd: number) => {
    ...
    return new Date(Date.UTC(targetYear, targetMonthIndex, clampedDay, 0, 0, 0, 0));
};

const nextOccurrenceUTC = (current: Date, frequency: ScheduleFrequency, interval: number) => {
    switch (frequency) {
        case ScheduleFrequency.DAILY:
            return new Date(Date.UTC(current.getUTCFullYear(), current.getUTCMonth(), current.getUTCDate() + interval, 0, 0, 0, 0));
        case ScheduleFrequency.WEEKLY:
            return new Date(Date.UTC(current.getUTCFullYear(), current.getUTCMonth(), current.getUTCDate() + 7 * interval, 0, 0, 0, 0));
...
export const createScheduleService = async (userId: number, dto: CreateScheduleDto) => {
    ...
    if (dto.budgetId) {
        const budget = await prisma.budget.findFirst({ where: { id: dto.budgetId, userId }, select: { id: true } });
        if (!budget) {
...
export const updateScheduleService = async (userId: number, id: number, dto: UpdateScheduleDto) => {
    const existing = await prisma.schedule.findFirst({
        where: { id, userId },
        select: { id: true, type: true, categoryId: true, startDate: true, endDate: true },
    });
    if (!existing) {
    ...
    const nextStart = dto.startDate ? toUTCDateOnly(new Date(dto.startDate)) : existing.startDate;
    const nextEnd =
    ...
    return prisma.schedule.update({
        ...
        data: {
            ...
            ...(dto.customInterval !== undefined ? { customInterval: dto.customInterval } : {}),
...
export const deleteScheduleService = async (userId: number, id: number) => {
    const existing = await prisma.schedule.findFirst({ where: { id, userId }, select: { id: true } });
    if (!existing) {
        const err: any = new Error("Schedule not found");
        err.statusCode = 404;
        throw err;
