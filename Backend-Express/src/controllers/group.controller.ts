...
class GroupController {
  ...
  static createExpense = async (req: Request, res: Response) => {
    try {
      ...
      const dto = (req as any).validated_body as CreateGroupExpenseDto;
      const expense = await createGroupExpenseService(userId, groupId, dto);
  ...
  static listExpenses = async (req: Request, res: Response) => {
    try {
      ...
      const expenses = await listGroupExpensesService(userId, groupId);
  ...
  static balances = async (req: Request, res: Response) => {
    try {
      const userId = (req as any).userId as number;
      const groupId = Number(req.params.id);
      if (!Number.isInteger(groupId)) {
      ...
      const data = await getGroupBalancesService(userId, groupId);
  ...
  static settlements = async (req: Request, res: Response) => {
    try {
      const userId = (req as any).userId as number;
      const groupId = Number(req.params.id);
      if (!Number.isInteger(groupId)) {
      ...
      const data = await getGroupSettlementsService(userId, groupId);
      return res.status(200).json(data);
    } catch (err: any) {
      const status = err?.statusCode ?? 500;
      return res
